...
class PoseMessage:
    ...
    def __str__(self):
        return f"Type: {self.message_type}, Time: {self.timestamp}, " \
               f"Position: {self.position}, Quaternion: {self.quaternion}, " \
               f"Velocity: {self.velocity}"

def initialize_serial_connection(port='/dev/ttyS0', baud_rate=115200, timeout=1):
    try:
...
def parse_and_print_message(message):
    ...
    try:
        ...
        _, message_type, timestamp, position_data, quaternion_data, velocity_data, received_chksum, _ = parts

        # Recompute the checksum for validation
        computed_chksum = calculate_checksum(';'.join(parts[:-2]))  # Excluding the received checksum and <EN> part
        ...
        pose_message = PoseMessage(message_type, timestamp, position, quaternion, velocity)
...
while True:
    if ser:
        try:            
            ...
            elif command_input == "z":  # Assuming 'z' is the trigger for the TEST command
                test_baud_rate(ser)
